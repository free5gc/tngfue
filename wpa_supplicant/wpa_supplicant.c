			     ...
			     const u8 *buf, size_t len)
{
	...
	if (!(wpa_s->drv_flags & WPA_DRIVER_FLAGS_4WAY_HANDSHAKE_PSK))
	{
		int ret = wpa_sm_rx_eapol(wpa_s->wpa, src_addr, buf, len);
#ifdef EAP_VENDOR_TEST
		if (ret == 2)
		{			
			wpa_printf(MSG_DEBUG, "finished! deinit current control interface");
			wpa_supplicant_ctrl_iface_deinit(wpa_s, wpa_s->ctrl_iface);
			wpa_printf(MSG_DEBUG, "Init new control interface");
			wpa_s->ctrl_iface = wpa_supplicant_ctrl_iface_init(wpa_s);
			wpa_supplicant_ikev2_conn(wpa_s->wpa);
		}
#endif
	}	
