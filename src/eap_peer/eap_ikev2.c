...
static int eap_ikev2_peer_keymat(struct eap_ikev2_data *data)
{
	if (eap_ikev2_derive_keymat(
		    data->ikev2.sa.proposal[0].prf, &data->ikev2.keys,
					   ...
					   struct eap_method_ret *ret, u8 id)
{
	...
	if (data->keys_ready) {
		...
		integ = ikev2_get_integ(data->ikev2.sa.proposal[0].integ);
		if (integ == NULL) {
	...
	if (flags & IKEV2_FLAGS_ICV_INCLUDED) {
		...
		ikev2_integ_hash(data->ikev2.sa.proposal[0].integ,
				 ...
				 int frag_ack)
{
	if (flags & IKEV2_FLAGS_ICV_INCLUDED) {
		int icv_len = eap_ikev2_validate_icv(
			data->ikev2.sa.proposal[0].integ, &data->ikev2.keys, 1,
			reqData, pos, *end);
		if (icv_len < 0)
			return -1;
