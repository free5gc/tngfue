			 ...
			 u8 next_payload, const u8 *pos, const u8 *end)
{
	...
	// notification data may have multiple information
	payloads->notification = (u8 **)malloc(0 * sizeof(u8 *));
	payloads->notification_len = (u8 *)malloc(0 * sizeof(u8));
	payloads->notification_num = 0;
	...
	while (next_payload != IKEV2_PAYLOAD_NO_NEXT_PAYLOAD) {
		...
		case IKEV2_PAYLOAD_NOTIFICATION:
			wpa_printf(MSG_DEBUG, "IKEV2:   Payload: "
				   "Notification");
			payloads->notification = realloc(payloads->notification, (payloads->notification_num + 1) * sizeof(u8 *));
			payloads->notification_len = realloc(payloads->notification_len, (payloads->notification_num + 1) * sizeof(u8));
			payloads->notification[payloads->notification_num] = pdata;
			payloads->notification_len[payloads->notification_num] = pdatalen;
			payloads->notification_num++;
			break;
		case IKEV2_PAYLOAD_TSi:
			wpa_printf(MSG_DEBUG, "IKEV2:   Payload: "
				   "TSi");
			payloads->tsi = pdata;
			payloads->tsi_len = pdatalen;
			break;
		case IKEV2_PAYLOAD_TSr:
			wpa_printf(MSG_DEBUG, "IKEV2:   Payload: "
				   "TSr");
			payloads->tsr = pdata;
			payloads->tsr_len = pdatalen;
			break;
		case IKEV2_PAYLOAD_CONFIGURATION:
			wpa_printf(MSG_DEBUG, "IKEV2:   Payload: "
				   "Configuration");
			payloads->configuration = pdata;
			payloads->configuration_len = pdatalen;
			break;
		default:
			  ...
			  struct wpabuf *plain, u8 next_payload)
{
	...
	wpa_hexdump_buf(MSG_DEBUG, "plain text is: ", plain);
			 ...
			 struct ikev2_keys *keys)
{
	...
	if (keys->SK_ei) {
		...
		wpa_hexdump(MSG_DEBUG, "IKEV2: SK_ei",
